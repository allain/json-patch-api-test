...
function Store(name, io) {
  ...
  this.update = function(updatedDoc, cb) {
    ...
    function processUpdate(err, snapshot) {
      ...
      if (diff.length === 0) {
        ...
        db.patches.insert({snapshotId: snapshot._id, ts: now, diff: diff}, updateSnapshotDoc);
      }
    }
    ...
    function updateSnapshotDoc(err, savedPatch) {
      ...
      db.snapshots.update({_id: snapshot._id}, {$set: {doc: updatedDoc, ts: now}}, buildResponse);
    }
  ...
  this.patches = function(after, cb) {
    ...
    function findSnapshotPatches(err, snapshot) {
      ...
      db.patches.find({snapshotId: snapshot._id, ts: {$gt: after}}).sort('ts', buildResponse);

    }

    function buildResponse(err, patches) {
      if (err) return cb(err);
